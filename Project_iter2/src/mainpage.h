 ...
 *
 * Another area of functionality is the Observer system. Both Packages and Drones extend EntityISubject,
 ...
 *
 * The last area of functionality is the Color Decorator system. The class DroneDecorator extends
 * Drone, and the class ColorDrone extends DroneDecorator. ColorDecorator has a Drone pointer
 * as a member variable, and it overrides most of Drone's methods to use the same methods on
 * its Drone member variable. ColorDrone's Update() method is overriden to call its member
 * drone's update, set its own position to match, and then--most importantly--update its
 * color based on its progress along the route. A Drone object can be converted to a ColorDrone
 * by creating a new ColorDrone and passing the old drone into its constructor; if the new
 * ColorDrone pointer is added to the list of entities instead of the old one, the drone will
 * update its color over time, and will otherwise act exactly like a normal drone.
 */
