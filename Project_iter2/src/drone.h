...
class Drone : public entity_project::Drone {
 public:
    ...
    */
     ...
     */
    virtual void SetRoute(std::vector<entity_project::SimpleUMNRoutePoint> newRoute);

    /**
     * @brief Set the drone's target package to an existing package.
     */
    virtual void SetPackage(Package* newPackage);

    /**
     * @brief Check if the drone's radius and another entity's radius overlap.
     */
    virtual bool CheckPosition(entity_project::Entity* other);

    /**
     * @brief Check if a SimpleUMNRoutePoint falls within the drone's radius.
     */
    virtual bool CheckPosition(entity_project::SimpleUMNRoutePoint other);
     ...
     */
    virtual bool GetIsBusy() { return isBusy; }

    /**
     * @brief Check if the drone is currently carrying a package.
     */
    virtual bool GetHasPackage() { return hasPackage; }
     ...
     */
    virtual const std::vector<std::string>& GetCurrentRoute() const { return *routeNew; }

    /**
     * @brief Get the current route as a vector of SimpleUMNRoutePoints
     */
    virtual std::vector<entity_project::SimpleUMNRoutePoint> GetRoute() { return route; }

    /**
     * @brief Get the remaining battery in seconds as a float.
     */
    /// TODO: implement this properly
    virtual float GetRemainingBattery() const { return max_batt; }

    /**
     * @brief Get current speed.
     */
    virtual float GetCurrentSpeed() const { return speed; }

    /**
     * @brief Get maximum package capacity.
     */
    /// TODO: implement this properly
    virtual float GetMaxCapacity() const { return capacity; }

    /**
     * @brief Get remaining package capacity.
     */
    /// TODO: implement this properly
    virtual float GetRemainingCapacity() const { return capacity - package->GetWeight(); }

    /**
     * @brief Get base acceleration
     */
    /// TODO: implement this properly
    virtual float GetBaseAcceleration() const { return accel; }

    /**
     * @brief Get route index
     */
    virtual int GetRouteIndex() const { return routeIndex; }
     ...
     */
    virtual void NotifyObservers(std::string task, entity_project::Entity *entity);
